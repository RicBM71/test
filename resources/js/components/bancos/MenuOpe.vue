    ...
    <div>
        ...
        <v-tooltip bottom>
            <template v-slot:activator="{ on }">
                <v-btn v-on="on" color="white" icon @click="goCreate">
        ...
        <v-tooltip bottom>
            <template v-slot:activator="{ on }">
                <v-btn v-show="id > 0" v-on="on" color="white" icon @click="openDialog">
...
export default {
    ...
    methods: {
        ...
        destroyReg() {
            ...
            axios
                .post('/mto/bancos/' + this.id, { _method: 'delete' })
                .then((response) => {
                    this.$router.push({ name: 'banco.index' });
                    this.$toast.success('Banco!');
                })
                .catch((err) => {
                    this.status = true;
