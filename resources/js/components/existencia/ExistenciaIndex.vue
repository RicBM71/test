        ...
        <v-container v-if="registros">
            ...
            <v-card>
                <v-card-title>
                    ...
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <v-btn v-show="items.length > 0 && hasExcel" v-on="on" color="white" icon @click="goExcel()">
                                <v-avatar size="32px">
                            ...
                            >
                                <template slot="items" slot-scope="props">
                                    ...
                                    <td class="justify-center layout px-0">
                                        <v-icon v-if="isRoot" small class="mr-2" @click="editItem(props.item.id)">
                                            edit
                                        </v-icon>
                                        <v-icon v-if="isRoot" small @click="openDialog(props.item.id)">
                                            delete
                                        </v-icon>
                                    </td>
                                </template>
...
export default {
    ...
    methods: {
        ...
        destroyReg() {
            ...
            axios
                .post(this.url + '/' + this.registro_id, { _method: 'delete' })
                .then((res) => {
                    if (res.status == 200) {
        ...
        goExcel() {
            ...
            axios({
                ...
                .then((response) => {
                    ...
                    link.download = 'Exi.' + new Date().getFullYear() + (new Date().getMonth() + 1) + new Date().getDate() + '.xlsx';
                ...
                .catch((err) => {
                    if (err.response.status == 403) this.$toast.error('No autorizado!');
            ...
            this.$validator.validateAll().then((result) => {
                if (result) {
                    ...
                    axios
                        ...
                        .catch((err) => {
                            this.show_loading = false;
                            if (err.response.status != 419) this.$toast.error(err.response.data.message);
                            else this.$toast.error('Sesión expirada!');
                        });
                }
            });
        },
