<template>
    <v-layout row justify-center>
        <v-dialog v-model="dialog" lazy persistent max-width="700px">
            <v-card>
                <v-card-title>
                <span class="headline">AMPLIAR CAPITAL</span>
                </v-card-title>
                <v-card-text>
                    <v-form>
                        <v-container>
                            <v-layout row wrap>
                                <v-flex sm3>
                                    ...
                                    >
                                        <v-text-field
                                            ...
                                            ></v-text-field>
                                        <v-date-picker
                                            v-model="deposito_capital.fecha"
                                            no-title
                                            :min="fecha_min"
                                            :max="hoy"
                                            locale="es"
                                ...
                                <v-flex sm5 d-flex>
                                    <v-select
                                        v-model="deposito_capital.concepto_id"
                                        :items="conceptos_com"
                                        label="Concepto"
                                        v-validate="'required'"
                               ...
                               <v-flex sm3>
                                   <v-text-field
                                        v-model="deposito_capital.importe"
...
export default {
    ...
    mounted(){
        axios.get(this.url)
            .then(res => {
                ...
                this.deposito_capital.concepto_id = this.conceptos_com[0].value;
            })
            .catch(err => {
    ...
    computed: {
        ...
        computedFecha() {
            moment.locale('es');
            return this.deposito_capital.fecha ? moment(this.deposito_capital.fecha).format('L') : '';
        },
    },
    methods:{
        closeDialog (){
        ...
        calImp(){
            ...
            this.deposito_capital.importe = Math.round(this.compra.importe_renovacion * (this.deposito_capital.dias / this.compra.dias_custodia),0);
        ...
        submit(){
            if (this.loading === false){
                ...
                this.$validator.validateAll().then((result) => {
                    if (result){
                        axios.post(this.url, this.deposito_capital)
                            .then(res => {
